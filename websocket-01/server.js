...
const initTranscoder = (sdp) => {
  const ffprocess = childProcess.spawn('ffmpeg', [
    ...
    '-maxrate', '1400k',
    '-bufsize', '600k',
    '-vf', 'scale=w=160:h=90:force_original_aspect_ratio=decrease',
    '-flags', '+cgop', '-g', '240', '-keyint_min', '240', // '-sc_threshold', '0',
    '-f', 'webm',
    // '-live',
    ...
    room.createRtpStreamer(producer, {
      ...
      .then(streamer => {
        ...
        ffprocess.stdout.on('data', chunk => {
          console.log('queueing chunk');
          bufList.push(chunk);

          if (bufList.length >= 20) {
            const chunks = bufList;

            bufList = [];

            console.log('sending chunks');
            socket.emit('media', ['data', Buffer.concat(chunks)]);
          }
        });
